...
import (
	...
	proto "bidstream_grpc/proto/chat" // 根据proto文件自动生成的代码
	"google.golang.org/grpc"
)

func main() {
	// 创建连接
	grpcConn, err := grpc.Dial("localhost:3000", grpc.WithInsecure())
	if err != nil {
		log.Printf("连接失败: [%v]\n", err)
		return
	}
	...
	// 声明客户端
	chatClient := proto.NewChatClient(grpcConn)
	...
	go func() {
		...
		for {
			...
			// 向服务端发送 指令
			if err := stream.Send(&proto.Request{Input: stdInput}); err != nil {
				return
			}
		}
	}()
